...
def parse_model_cfg(path: str):
    ...
    # 去除空行和注释行，去除每行开头和结尾的空格符
    lines = [x for x in lines if x and not x.startswith("#")]
    lines = [x.strip() for x in lines]

    # 遍历文件，把有用的层结构信息保存在一个列表mdefs里
    mdefs = []  # module definitions，字典的列表
    for line in lines:
        if line.startswith("["):  # this marks the start of a new block
            mdefs.append({})  # 给模型添加一个字典
            ...
            if key == "anchors":  # 如果key为anchors的话，就是yolo层
                ...
                if val.isnumeric():  # return int or float 如果是数值的情况
                    mdefs[-1][key] = int(val) if (int(val) - float(val)) == 0 else float(val)  # 判断是什么数据类型
                else:
